  ...
  return (
    <motion.ul
      ...
      className={`${clicked ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"} 
        absolute lg:relative top-20 lg:top-0 left-0 flex flex-col lg:flex-row items-center justify-end 
        lg:justify-center space-y-6 lg:space-y-0 lg:space-x-2 w-full lg:w-auto lg:h-full py-12 lg:py-0 
      bg-grey lg:bg-inherit text-2xl md:text-lg font-sans font-medium text-white 
        transition-all origin-top lg:scale-y-100 lg:opacity-100`}
    >
      {navLinks.map((link) => (
